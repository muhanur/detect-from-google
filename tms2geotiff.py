...
def draw_tile(source, lat0, lon0, lat1, lon1, zoom, filename):
    ...
    #print('Saving GeoTIFF file: {0}...'.format(filename))
    img = finish_picture(bigim, base_size, bbox, x0, y0, x1, y1)
    imgbands = len(img.getbands())
    driver = gdal.GetDriverByName('GTiff')
    gtiff = driver.Create(filename, img.size[0], img.size[1], imgbands, gdal.GDT_Byte, options=['COMPRESS=DEFLATE', 'PREDICTOR=2', 'ZLEVEL=9', 'TILED=YES'])
